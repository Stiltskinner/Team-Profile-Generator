...
const genManager = (data) => {
    const { name, id, email, officeNum, addAnother } = data
    const newManager = new Manager(name, id, email, officeNum, addAnother);
    formatManager(newManager);
}

const formatManager = (element) => {
    let managerEl = element;
    const { name, id, email, officeNum, addAnother } = managerEl;
